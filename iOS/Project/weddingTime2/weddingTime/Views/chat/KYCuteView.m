...
-(void)setUp{
    ...
    if (!self.frontView) {
        self.frontView = [[UIButton alloc]initWithFrame:CGRectMake(initialPoint.x,initialPoint.y, self.bubbleWidth, self.bubbleHeight)];
    }
        ...
        else
        {
            [UIView animateWithDuration:0.5 delay:0.0f usingSpringWithDamping:0.4f initialSpringVelocity:0.0f options:UIViewAnimationOptionCurveEaseInOut animations:^{
                ...
                if (finished) {
                    ...
                    self.frontView.center = oldBackViewCenter;
                    if (self.delegate&&[self.delegate respondsToSelector:@selector(clear)]) {
                        [self.delegate clear];
                    }
                }
            }];
...
@implementation UIView (KYCuteView)
- (KYCuteView *)smallBadgeViewWithViewWidth:(CGFloat)width
{
	KYCuteView *badgeView = [[KYCuteView alloc] initWithPoint:CGPointMake(width - 12, -3) superView:self];
	badgeView.viscosity  = 20;
	badgeView.bubbleWidth = 12;
	badgeView.bubbleHeight = 12;
	[badgeView setUp];
	badgeView.bubbleLabel.backgroundColor=[WeddingTimeAppInfoManager instance].baseColor;
	badgeView.bubbleLabel.font=[WeddingTimeAppInfoManager fontWithSize:14];
	badgeView.bubbleLabel.adjustsFontSizeToFitWidth = YES;
	badgeView.bubbleLabel.layer.cornerRadius = 6;
	badgeView.bubbleLabel.layer.masksToBounds=YES;

	return badgeView;
}

- (KYCuteView *)badgeViewWithViewWidth:(CGFloat)width
{
	KYCuteView *badgeView = [[KYCuteView alloc] initWithPoint:CGPointMake(width - 22, 0) superView:self];
	badgeView.viscosity  = 20;
	badgeView.bubbleWidth = 22;
	badgeView.bubbleHeight = 22;
	[badgeView setUp];
	badgeView.bubbleLabel.backgroundColor=[WeddingTimeAppInfoManager instance].baseColor;
	badgeView.bubbleLabel.font=[WeddingTimeAppInfoManager fontWithSize:14];
	badgeView.bubbleLabel.adjustsFontSizeToFitWidth = YES;
	badgeView.bubbleLabel.layer.cornerRadius=11;
	badgeView.bubbleLabel.layer.masksToBounds=YES;

	return badgeView;
}

- (KYCuteView *)baseBadgeViewWithViewWidth:(CGFloat)width
{
	KYCuteView *badgeView = [[KYCuteView alloc] initWithPoint:CGPointMake(width - 22 - 10, 0) superView:self];
	badgeView.viscosity  = 20;
	badgeView.bubbleWidth = 16;
	badgeView.bubbleHeight = 16;
	[badgeView setUp];
	badgeView.bubbleLabel.backgroundColor = WeddingTimeBaseColor;
	badgeView.bubbleLabel.textColor = WHITE;
	badgeView.bubbleLabel.font = [WeddingTimeAppInfoManager fontWithSize:11];
	badgeView.bubbleLabel.adjustsFontSizeToFitWidth = YES;
	badgeView.bubbleLabel.layer.cornerRadius = 8;
	badgeView.bubbleLabel.layer.masksToBounds=YES;
