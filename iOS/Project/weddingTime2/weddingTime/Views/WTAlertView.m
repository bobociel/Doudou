...
const static CGFloat kCustomIOSBorderTop                = 20.0;
const static CGFloat kCustomIOSBorderBottom             = 30.0;
const static CGFloat kCustomIOSBorderCenter             = 20.0;
...
- (instancetype)initWithText:(NSString*)title centerImage:(UIImage*)image
{
    self = [self init];
    if (self) {
		self.tag = UIWindowLevelAlert;
		
        UILabel *label= [[UILabel alloc]initWithFrame:CGRectMake(0, 0, 300, 18)];
        label.text=title;
        label.textAlignment=NSTextAlignmentCenter;
        label.textColor=[UIColor colorWithRed:102.0/255 green:102.0/255 blue:102.0/255 alpha:1];
        label.font=[WeddingTimeAppInfoManager fontWithSize:18];
        label.numberOfLines=0;
        label.size=[label sizeThatFits:CGSizeMake(screenWidth /3 *2, CGFLOAT_MAX)];
    
        UIView *view=[[UIView alloc]initWithFrame:CGRectMake(0, 0, screenWidth /6 *5, label.height+kCustomIOSBorderTop+kCustomIOSBorderBottom)];
        ...
        else
        {
            ...
            UIImageView *imageview=[[UIImageView alloc]initWithFrame:CGRectMake(view.width/2-imageWidth/2, 0, imageWidth,imageWidth)];
            imageview.layer.cornerRadius = imageWidth / 2.0;
            imageview.layer.masksToBounds = YES;
            imageview.image=image;
            imageview.top=kCustomIOSBorderTop;
            
            label.top=imageview.bottom+kCustomIOSBorderCenter;
            
            view.height+=imageview.height+kCustomIOSBorderCenter;
...
- (void)show
{
	if([KeyWindowCurrentViewController isKindOfClass:[UIViewController class]]){
		[KeyWindowCurrentViewController.view endEditing:YES];
	}
...
- (void)customIOS7dialogButtonTouchUpInside: (WTAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
...
- (UIView *)createContainerView
{
    if (containerView == NULL) {
        containerView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, screenWidth, 150)];
    }
    ...
    lineView.backgroundColor = [UIColor colorWithRed:228/255.0 green:228/255.0 blue:228/255.0 alpha:1.0f];
...
- (void)addButtonsToView: (UIView *)container
{
    ...
    for (int i=0; i<[buttonTitles count]; i++) {
        ...
        [closeButton.titleLabel setFont:[WeddingTimeAppInfoManager fontWithSize:20]];
        [closeButton.layer setCornerRadius:kCustomIOSAlertViewCornerRadius];

        UIColor *color = [buttonTitles[i] isEqualToString:CancelString] ? [UIColor colorWithRed:0.2f green:0.2f blue:0.2f alpha:0.5f] : [WeddingTimeAppInfoManager instance].baseColor ;
...
//    CGFloat screenWidth_wd = [UIScreen mainScreen].bounds.size.width;
//    CGFloat screenHeight_wd = [UIScreen mainScreen].bounds.size.height;

    // On iOS7, screen width and height doesn't automatically follow orientation
    if (floor(NSFoundationVersionNumber) <= NSFoundationVersionNumber_iOS_7_1) {
        UIInterfaceOrientation interfaceOrientation = [[UIApplication sharedApplication] statusBarOrientation];
        if (UIInterfaceOrientationIsLandscape(interfaceOrientation)) {
//            CGFloat tmp = screenWidth_wd;
//            screenWidth_wd = screenHeight_wd;
//            screenHeight_wd = tmp;
        }
    }
...
- (void)changeOrientationForIOS8: (NSNotification *)notification {
    ...
    [UIView animateWithDuration:0.2f delay:0.0 options:UIViewAnimationOptionTransitionNone
                     animations:^{
                         ...
                         self.frame = CGRectMake(0, 0, screenWidth_wd, screenHeight_wd);
                         dialogView.frame = CGRectMake((screenWidth_wd - dialogSize.width) / 2, (screenHeight_wd - keyboardSize.height - dialogSize.height) / 2, dialogSize.width, dialogSize.height);
                     }
                     completion:nil
     ];
