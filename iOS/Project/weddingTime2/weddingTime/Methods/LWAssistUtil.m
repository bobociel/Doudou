...
+ (NetWorkStatusType)getNetWorkStatusType:(NSError *)error
{
    ...
    {
        if (error.userInfo) {
            if ([error.userInfo[@"status"] intValue] == 1038 || [error.userInfo[@"status"] intValue] == 403 ) {
...
+ (NSArray *)defaultCategoryString
{
	return @[@"婚礼策划",@"婚纱写真",@"婚礼跟拍",@"婚礼主持",@"婚纱礼服",@"新娘跟妆",@"婚礼摄像"];
}

+ (NSDictionary *)defaultCategory
{
	return  @{@(WTWeddingTypePlan).stringValue:@"婚礼策划",
			  @(WTWeddingTypePhoto).stringValue:@"婚纱写真",
			  @(WTWeddingTypeCapture).stringValue:@"婚礼跟拍",
			  @(WTWeddingTypeHost).stringValue:@"婚礼主持",
			  @(WTWeddingTypeDress).stringValue:@"婚纱礼服",
			  @(WTWeddingTypeMakeUp).stringValue:@"新娘跟妆",
			  @(WTWeddingTypeVideo).stringValue:@"婚礼摄像",
			  @(WTWeddingTypeHotel).stringValue:@"婚宴酒店"};
}
