...
#import "LWUtil.h"
#define kMaxThumbWidth  1000.0
#define kMaxThumbHeight 1000.0
...
+ (CGSize)textViewSizeWithTextView:(UITextView *)textView andWidth:(CGFloat)width
{
	return [textView sizeThatFits:CGSizeMake(width, MAXFLOAT)];
}

+ (CGSize)sizeWiithString:(NSString *)str andAttribute:(NSDictionary *)attribute andWidth:(CGFloat)width
{
	NSString *string = [NSString stringWithFormat:@"/n%@",str];
...
+ (NSString *)getDateSringCYMD:(NSDate *)date {
	NSDateFormatter *formatter = [[NSDateFormatter alloc] init] ;
...
+ (UIImage *)scaleUploadImage:(UIImage *)image
{
	CGFloat wScale = image.size.width / kMaxThumbWidth ;
	CGFloat hScale = image.size.height / kMaxThumbHeight;
	CGFloat toScale = MAX(wScale, hScale);
	CGSize toSize = CGSizeMake(image.size.width / toScale, image.size.height / toScale);
	if(wScale > 1 || hScale > 1)
	{
		return [self OriginImage:image scaleToSize:toSize];
	}
	else
	{
		return image;
...
+ (NSString *)getVIPNameWithSupplierLeve:(WTSupplierLevel)level
{
	NSString *VIPName = @"";
	if(level == WTSupplierLevelV){
		VIPName = @"官方婚礼顾问";
	}else if (level == WTSupplierLevelV0){
		VIPName = @"";
	}
	else if (level == WTSupplierLevelV1){
		VIPName = @"合作";
	}
	else if (level == WTSupplierLevelV2){
		VIPName = @"认证";
	}else if (level == WTSupplierLevelV3){
		VIPName = @"品牌";
	}
	return VIPName;
}

+ (NSString *)getVIPComDetailWithSupplierLeve:(WTSupplierLevel)level andTypeName:(NSString *)typename
{
	NSString *VIPName = @"";
	if(level == WTSupplierLevelV ){
		VIPName = [LWUtil getVIPNameWithSupplierLeve:level];
	}else if(level == WTSupplierLevelV0){
		VIPName = [LWUtil getString:typename andDefaultStr:@""];
	}else{
		VIPName = [NSString stringWithFormat:@"%@ | %@",[LWUtil getVIPNameWithSupplierLeve:level],[LWUtil getString:typename andDefaultStr:@""]];
	}
	return VIPName;
}

+ (NSString *)getVIPComDescWithSupplierLeve:(WTSupplierLevel)level andTypeName:(NSString *)typename andComany:(NSString *)company
{
	NSString *VIPName = @"";
	if(level == WTSupplierLevelV){
		VIPName = [NSString stringWithFormat:@"%@：%@",[LWUtil getVIPNameWithSupplierLeve:level],
				   [LWUtil getString:company andDefaultStr:@""]] ;
	}else{
		VIPName = [NSString stringWithFormat:@"%@%@：%@",[LWUtil getVIPNameWithSupplierLeve:level],
				   [LWUtil getString:typename andDefaultStr:@""],
				   [LWUtil getString:company andDefaultStr:@""]];
	}
	return VIPName;
}

+ (UIImage *)getVIPImageWithSupplierLevel:(WTSupplierLevel)level
{
	UIImage *VIPImage = [UIImage imageNamed:@""];
	if(level == WTSupplierLevelV){
		VIPImage = [UIImage imageNamed:@"icon_v"];
	}
	else if (level == WTSupplierLevelV1){
		VIPImage = [UIImage imageNamed:@"icon_v1"];
	}
	else if (level == WTSupplierLevelV2){
		VIPImage = [UIImage imageNamed:@"icon_v2"];
	}else if (level == WTSupplierLevelV3){
		VIPImage = [UIImage imageNamed:@"icon_v3"];
	}
	return VIPImage;
}
...
+ (NSString *)StringWithCoupon:(NSString *)coupon
{
	NSString *priceS = [NSString stringWithFormat:@"  婚礼宝补贴:￥%@起  ",[LWUtil getString:coupon andDefaultStr:@"0"]];

	return priceS;
}

+ (NSMutableAttributedString *)attributeStringWithPrice:(NSString *)price
{
	NSString *priceS = [NSString stringWithFormat:@"￥%@起",price];
	NSMutableAttributedString *attStr = [[NSMutableAttributedString alloc] initWithString:priceS];
	NSRange dotRange = [priceS rangeOfString:@"."];

	if(dotRange.location != NSNotFound){
		[attStr addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:21.0] range:NSMakeRange(1, dotRange.location)];
	}

	return attStr;
}

+ (NSMutableAttributedString *)attributeStringWithStartPrice:(NSString *)price
{
    NSString *priceS = [NSString stringWithFormat:@"￥%@起",price];
    NSMutableAttributedString *attStr = [[NSMutableAttributedString alloc] initWithString:priceS];
    [attStr addAttribute:NSFontAttributeName value:DefaultFont10 range:NSMakeRange(priceS.length-1, 1)];
...
{
	NSString *priceS = [NSString stringWithFormat:@"￥%@",[LWUtil getString:price andDefaultStr:@"0"]];
	return priceS;
}

+ (NSString *)stringWithStartPrice:(NSString *)price
{
	NSString *priceS = @"暂无报价";
	if([price isNotEmptyCtg]){
		priceS = [NSString stringWithFormat:@"￥%@起",[LWUtil getString:price andDefaultStr:@""]];
	}
	return priceS;
}

+ (NSString *)stringWithCompany:(NSString *)company andCity:(NSString *)city andPrice:(NSString *)price
{
	NSString *string = [NSString stringWithFormat:@"by %@ | %@ | %@",
						[LWUtil getString:company andDefaultStr:@""],
						[LWUtil getString:city andDefaultStr:@"全国"],
						[LWUtil stringWithStartPrice:price]];
	return string;
}

+ (NSMutableAttributedString *)attributeStringWithBudget:(NSString *)price
{
	NSString *priceS = [NSString stringWithFormat:@"￥%@",price];
	NSMutableAttributedString *attStr = [[NSMutableAttributedString alloc] initWithString:priceS];
	[attStr addAttribute:NSFontAttributeName value:DefaultFont14 range:NSMakeRange(0,1)];
	return attStr;
}

+ (NSMutableAttributedString *)attributeStringWithAllBudget:(NSString *)price
{
	NSString *priceS = [NSString stringWithFormat:@"%@元",price];
	NSMutableAttributedString *attStr = [[NSMutableAttributedString alloc] initWithString:priceS];
	NSRange dotRange = [priceS rangeOfString:@"元"];
	[attStr addAttribute:NSFontAttributeName value:DefaultFont14 range:NSMakeRange(dotRange.location,1)];
	return attStr;
}

+ (NSMutableAttributedString *)attributeStringWithType:(FindItemType)type andContent:(NSString *)content
{
	NSString *contents;
	UIFont *textFont;
	NSRange textRange = NSMakeRange(0, 0);
	if(type == FindItemTypeSupplier || type == FindItemTypePost){
		textFont = [UIFont boldSystemFontOfSize:14];
		contents = [NSString stringWithFormat:@"by %@",content];
		textRange = [contents rangeOfString:@"by"];
	}else if (type == FindItemTypeInspiretion){
		textFont = [UIFont boldSystemFontOfSize:19];
		contents = [NSString stringWithFormat:@"%@ 个婚礼灵感",content];
		textRange = [contents rangeOfString:content];
	}

	NSMutableAttributedString *attStr = [[NSMutableAttributedString alloc] initWithString:[LWUtil getString:contents andDefaultStr:@""]];
	[attStr addAttribute:NSFontAttributeName value:textFont range:textRange];
	return attStr;
}

+ (NSMutableAttributedString *)attributeStringWithPriceRange:(NSString *)priceRange
{
	NSString *priceS = [NSString stringWithFormat:@"预算 ￥%@",priceRange];
	NSMutableAttributedString *attStr = [[NSMutableAttributedString alloc] initWithString:priceS];
	NSRange dollRange = [priceS rangeOfString:@"￥"];

	[attStr addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:18.0] range:NSMakeRange(dollRange.location, priceS.length - 3)];
	[attStr addAttribute:NSForegroundColorAttributeName value:WeddingTimeBaseColor range:NSMakeRange(dollRange.location, priceS.length - 3)];

	return attStr;
}

+ (NSMutableAttributedString *)attributeStringWithdrawPrice:(NSString *)price
{
	NSString *priceS = [NSString stringWithFormat:@"可提现金    %@",[LWUtil stringWithPrice:price]];
	NSMutableAttributedString *priceAttrS = [[NSMutableAttributedString alloc] initWithString:priceS];
	[priceAttrS addAttributes:@{NSForegroundColorAttributeName:rgba(183, 183, 183, 1)}
						range:NSMakeRange(0, 4)];
	[priceAttrS addAttributes:@{NSForegroundColorAttributeName:WeddingTimeBaseColor,
								NSFontAttributeName:DefaultFont18} range:NSMakeRange(8, priceS.length - 8)];
	return priceAttrS;
}
...
+ (NSString *)getString:(id)info andDefaultStr:(NSString *)defaultStr
{
	...
	{
		return [info length] > 0 ? info : defaultStr ;
    }
...
+ (NSNumber *)getNumber:(id)info
{
	...
	if ([info isKindOfClass:[NSString class]])
	{
		return [NSNumber numberWithString:info] ;
	}
	else if ([info isKindOfClass:[NSNumber class]])
	{
		return info;
