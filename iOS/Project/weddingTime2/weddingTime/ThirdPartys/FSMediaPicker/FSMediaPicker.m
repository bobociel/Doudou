...
- (void)delegatePerformFinishWithMediaInfo:(NSDictionary *)mediaInfo andPicker:(UIImagePickerController *)picker;
...
{
    [self delegatePerformFinishWithMediaInfo:info andPicker:picker];
}
...
{
    if ([viewController isKindOfClass:NSClassFromString(@"PLUIImageViewController")]&& [navigationController.viewControllers count] == 3 )
    {
        UIView *plCropOverlay = [[viewController.view.subviews objectAtIndex:1] subviews][0];
        plCropOverlay.hidden = NO;
        plCropOverlay.frame=[UIScreen mainScreen].bounds;
        if (self.editMode == FSEditModeCircular) {
            ...
            int position = screenHeight/2-diameter/2;
            ...
            if (!isIpad) {
                ...
                [moveLabel setText:kMoveAndScale];
                [moveLabel setTextAlignment:NSTextAlignmentCenter];
                [moveLabel setTextColor:[UIColor whiteColor]];
                [viewController.view addSubview:moveLabel];
            }
        }
        else if (self.editMode == FSEditModeBackGround)
        {
            plCropOverlay.hidden = YES;
...
- (void)delegatePerformFinishWithMediaInfo:(NSDictionary *)mediaInfo andPicker:(UIImagePickerController *)picker
{
    ...
    if(([AFNetworkReachabilityManager sharedManager].networkReachabilityStatus != AFNetworkReachabilityStatusReachableViaWiFi) && self.mediaType == FSMediaTypeVideo){
        WTAlertView *alertView = [[WTAlertView alloc]initWithText:@"当前网络非wifi环境是否继续上传视频" centerImage:nil];
...
- (void)setBarWIth:(UIImagePickerController *)imagePicker
{
    [imagePicker.navigationBar setShadowImage:[[UIImage alloc] init]];
    [imagePicker.navigationBar setBackgroundImage:[LWUtil imageWithColor:[UIColor whiteColor]] forBarMetrics:UIBarMetricsDefault];
    [imagePicker.navigationBar setTintColor:[WeddingTimeAppInfoManager instance].baseColor];
    [imagePicker.navigationBar setTitleTextAttributes:@{NSFontAttributeName:[WeddingTimeAppInfoManager fontWithSize:18],NSForegroundColorAttributeName:[UIColor blackColor]}];
}
