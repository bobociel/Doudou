...
@property (weak, nonatomic) IBOutlet UIButton *doFavBtn;
...
+(void)show:(UIViewController *)handleVC needFav:(BOOL)needFav type:(PhotoBroswerVCType)type index:(NSUInteger)index photoModelBlock:(NSArray *(^)())photoModelBlock{
    ...
    pbVC.navHidden= handleVC.navigationController.navigationBarHidden;
    //记录
    pbVC.type =type;
    ...
    if (!needFav) {//目前在灵感下才会点赞
        pbVC.doFavBtn.hidden=YES;
    }
    else
    {
        pbVC.doFavBtn.hidden=NO;
    }
}
...
-(void)vcPrepare{
    ...
    self.doFavBtn.layer.shadowColor=[UIColor blackColor].CGColor;
    self.doFavBtn.layer.shadowOpacity=1;
    self.doFavBtn.layer.shadowOffset=CGSizeMake(-1, 1);
    self.doFavBtn.layer.shadowRadius=1;
    
    self.topBarLabel.layer.shadowColor=[UIColor blackColor].CGColor;
    self.topBarLabel.layer.shadowOpacity=1;
    self.topBarLabel.layer.shadowOffset=CGSizeMake(-1, 1);
    self.topBarLabel.layer.shadowRadius=1;
    
    self.SaveButton.layer.shadowColor=[UIColor blackColor].CGColor;
    self.SaveButton.layer.shadowOpacity=1;
    self.SaveButton.layer.shadowOffset=CGSizeMake(-1, 1);
    self.SaveButton.layer.shadowRadius=1;
    ...
    self.BackButton.layer.shadowRadius=1;
...
-(void)showWithPage:(NSUInteger)page{
	...
	[photoItemView setItemViewTitleTapBlock:^(NSString *postID) {
		dispatch_async(dispatch_get_main_queue(), ^{
			WTWorksDetailViewController *worksVC = [WTWorksDetailViewController instanceWTWorksDetailVCWithWrokID:postID];
...
-(void)setPage:(NSUInteger)page{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if ([self.currentItemView.photoModel.delegate respondsToSelector:@selector(setBtn::)]) {
            [self.currentItemView.photoModel.delegate setBtn:(int)self.currentItemView.photoModel.mid :self.doFavBtn];
        }
    });
}
...
- (IBAction)doFav:(id)sender {
    if ([self.currentItemView.photoModel.delegate respondsToSelector:@selector(doFav::)]) {
    [self.currentItemView.photoModel.delegate doFav:(int)self.currentItemView.photoModel.mid :self.doFavBtn];
    }
}
