...
/** 点击标题*/
@property (nonatomic, strong) UITapGestureRecognizer *tapTitle;
...
-(void)awakeFromNib{
    ...
    self.descLabel.layer.shadowOpacity=1;
    self.descLabel.layer.shadowOffset=CGSizeMake(-1, 1);
    self.descLabel.layer.shadowRadius=1;
	...
	[self.titleLabel addGestureRecognizer:self.tapTitle];
}
...
- (void)tap_title{
	if(_ItemViewTitleTapBlock != nil) {
		_ItemViewTitleTapBlock(self.photoModel.postID);
	}
}
...
-(void)setGradient
{
    ...
    for (int i=7; i>=0; i--) {
        ...
        CGColorRef color=[UIColor colorWithRed:0 green:0 blue:0 alpha:0.1].CGColor;
        [array addObject:(__bridge id)color];
    }
    for (int i=0; i<8; i++) {
        CGColorRef color=[UIColor colorWithRed:0 green:0 blue:0 alpha:0.1].CGColor;
        [array addObject:(__bridge id)color];
    ...
    [self.layer insertSublayer:gradient below:self.bottomView.layer];
}
 ...
 *点击标题
 */
- (UITapGestureRecognizer *)tapTitle
{
	if(!_tapTitle){
		_tapTitle = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(tap_title)];
	}
	return _tapTitle;
}
