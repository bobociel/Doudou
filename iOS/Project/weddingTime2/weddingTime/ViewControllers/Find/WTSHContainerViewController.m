...
@property (nonatomic, copy) NSString *su_syn_id;
@end
...
- (void)viewDidLoad {
	...
	self.filters = [[WTHotelFilters alloc] init];
...
- (void)topView:(WTTopView *)topView didSelectedFilter:(UIControl *)likeButton
{
	if(_topView.segmentCon.selectedSegmentIndex == WTSegmentTypePost){
		...
		WTFilterHotelViewController *hotelFilter = [WTFilterHotelViewController instanceVCWithFilters:_filters];
		...
		synFilter.synOrCityID = _su_syn_id;
		[self.navigationController presentViewController:synFilter animated:YES completion:nil];
	}
}
...
- (void)synFilterHasSelectWithInfo:(id)info index:(NSIndexPath *)index
{
	if(_topView.segmentCon.selectedSegmentIndex == WTSegmentTypePost){
		_syn_id = info;
	}else if (_topView.segmentCon.selectedSegmentIndex == WTSegmentTypeSupplier){
		_su_syn_id = info;
	}
	[self filterData];
}
	...
	if(_topView.segmentCon.selectedSegmentIndex == WTSegmentTypePost)
	{
		[_postViewController filterDataWithSType:_supplier_type
										 segType:_topView.segmentCon.selectedSegmentIndex
										   synID:_syn_id
										 filters:_filters];
		[_postViewController.view removeFromSuperview];
		[self.view insertSubview:_postViewController.view belowSubview:_topView];
	}
	else if (_topView.segmentCon.selectedSegmentIndex == WTSegmentTypeSupplier)
	{
		[_suppViewController filterDataWithSType:_supplier_type
										 segType:_topView.segmentCon.selectedSegmentIndex
										   synID:_su_syn_id
										 filters:_filters];
		[_suppViewController.view removeFromSuperview];
		[self.view insertSubview:_suppViewController.view belowSubview:_topView];
	}
}
