...
@interface WTFindViewController ()<UITableViewDataSource, UITableViewDelegate, FilterSelectDelegate,WTTopViewDelegate>
...
- (void)loadCityFilterData
{
    WS(ws);
    [NetWorkingFailDoErr removeAllErrorViewAtView:self.tableView];
    city_id = @([UserInfoManager instance].curCityId).stringValue;
    self.array = [NSMutableArray array];
    int64_t nowDate = [[NSDate date] timeIntervalSince1970];
    [self loadLocalDataWithCityId:city_id Time:nowDate limit:10];
...
- (void)cateAction:(UIButton *)cateBtn
{
	...
	[PostDataService postAccessLogWithServiceID:type andCityID:nil andID:nil andLogType:WTLogTypeNone];
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
	if([scrollView isEqual:_tableView]){
		...
		scrollViewFrame.origin.y = MIN(scrollViewFrame.origin.y, screenHeight);
		scrollViewFrame.origin.y = MAX(scrollViewFrame.origin.y, kScrollViewY);
