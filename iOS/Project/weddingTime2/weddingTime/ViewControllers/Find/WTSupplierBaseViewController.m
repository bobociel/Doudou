        ...
        [ChatConversationManager getConversationWithClientIds:ids type:type withBlock:^(AVIMConversation *conversation, NSError *error){
            ...
            else
            {
                ...
                if (sendToAskData && [sendToAskData isKindOfClass:[NSDictionary class]]){
					NSString *avatarStr = type == WTConversationTypePost ? sendToAskData[@"post_cover"] : avatar;
					...
					[sendData setObject:(avatarStr ? : @"") forKey:@"avatar"];
                    next.sendToAskData = sendData;
                }
				else if([sendToAskData isKindOfClass:[NSString class]] && [LWUtil getString:sendToAskData andDefaultStr:@""].length > 0){
