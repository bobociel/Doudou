...
-(void)viewWillAppear:(BOOL)animated
{
	...
	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(loginWXWithResponse:) name:UserDidLoginWithWX object:nil];
}
...
- (void)loginWXWithResponse:(NSNotification *)noti
{
	[self showLoadingViewTitle:@"登录中"];
	SendAuthResp *authResp = [SendAuthResp modelWithJSON:noti.userInfo[@"data"]];
	[UserService getWXUserInfoWithCode:authResp.code callback:^(NSDictionary *result, NSError *error){
		[self hideLoadingView];
		if(!error)
		{
			if([LWUtil getString:result[@"data"][@"phone"] andDefaultStr:@""].length == 0)
			{
				[self.navigationController pushViewController:[WTBindingViewController instanceWTBindingViewControllerWithToken:result[@"data"][@"token"]] animated:YES];
			}
			else
			{
				[UserInfoManager instance].tokenId_self = result[@"data"][@"token"];
		...
		{
			[WTProgressHUD ShowTextHUD:@"登录失败" showInView:nil];
		}
	}];
}
...
- (void)loginSucceed:(NSDictionary*)result {
	...
	else if( [LWUtil getString:result[@"phone"] andDefaultStr:@""].length == 0)
	{
		[UserInfoManager instance].noPSW = YES;
		[self.navigationController pushViewController:[WTBindingViewController instanceWTBindingViewControllerWithToken:result[@"token"]] animated:YES];
	}
	else
	{
