...
-(void)sendFirstMsg
{
    [self sendConversationEmail];
    [self sendTellPartener];
}

//发送Email
-(void)sendConversationEmail
{
	[PostDataService postSendEmailBlock:^(NSDictionary *result, NSError *error) {
...
-(void)sendTellPartener
{
	...
	NSString *serverId = [LWUtil getString:partnerId andDefaultStr:@""];
    NSString *name = partnerUserInfo.name;
	NSString *avatar = [[SQLiteAssister sharedInstance] pullUserInfo:serverId].avatar ?: @"";
    if (self.sendToAskData) {
