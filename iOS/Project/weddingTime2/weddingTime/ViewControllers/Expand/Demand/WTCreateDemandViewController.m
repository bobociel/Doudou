...
-(void)initView{
    ...
    serviceTypePickArr =@[@"婚礼策划",@"婚纱写真",@"婚礼跟拍",@"婚礼摄像",@"婚纱礼服",@"新娘跟妆",@"婚礼主持",@"甜品婚品",@"海外婚礼"];
	...
	NSString *weddingTime = [UserInfoManager instance].weddingTime > 0 ? [[UserInfoManager instance] weddingTimeString ] :@"请选择婚期" ;
...
-(void)chooseService
{
    pickView= [CommPickView instanceWithStyle:PickViewStylePicker
									   andTag:WTChooseTypeService
									 andArray:serviceTypePickArr];
...
- (void)didPickObjectWithIndex:(int)index andTag:(NSInteger)tag{
    ...
    if (tag == WTChooseTypePrice){
        
        [choosePriceBtn setTitle:priceArray[index] forState:UIControlStateNormal];
        [choosePriceBtn setTitleColor:WeddingTimeBaseColor forState:UIControlStateNormal];
        choosePriceBtn.titleLabel.font=DefaultFont16;

        priceRangeIndex=index;
        
    }
    if (tag == WTChooseTypeCity){
...
-(void)commiEvent{
    ...
    else if ([choosePriceBtn.titleLabel.text isEqualToString:@"请选择价位"] || priceRangeIndex == -1)
	{
	...
	else if ([chooseWeddingTime.titleLabel.text isEqualToString:@"请选择婚期"] || weddingDay_time <= 0 )
	{
	 ...
	 else if ([chooseCityBtn.titleLabel.text isEqualToString:@"请选择婚礼举办城市"] || cityIndex <= -1)
	 {
