						...
						avatar:(NSString *)avatar
{
    ...
    if ([serverId isNotEmptyCtg] && [[UserInfoManager instance].userId_self isNotEmptyCtg])
	{
		...
		if(type == WTConversationTypeSupplier || type == WTConversationTypePost )
		{
			...
			[ids addObject:kServerID];
		}
		else if (type == WTConversationTypeHotel)
		{
        ...
        [ChatConversationManager getConversationWithClientIds:ids type:type withBlock:^(AVIMConversation *conversation, NSError *error,BOOL iscreated){
            ...
            else
            {
                ...
                if (sendToAskData)
				{
					...
					[sendData setObject:type == WTConversationTypeHotel ? @"h" : (type == WTConversationTypeSupplier) ? @"s" : @"p" forKey:@"type"];
					[sendData setObject:serverName forKey:@"name"];
					[sendData setObject:avatar forKey:@"avatar"];
