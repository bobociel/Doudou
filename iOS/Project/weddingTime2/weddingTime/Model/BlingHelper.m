...
{
    if(![[UserInfoManager instance].userId_partner isNotEmptyCtg]&& [[UserInfoManager instance].userId_self isNotEmptyCtg] ) {
        [BlingService getInviteStateWithBlock:^(NSDictionary *result, NSError *error) {
            ...
            if ([type isEqualToString:@"invitee"]) {
                ...
                [helper makeAlertWith:result type:1 callback:callback];
            }else{
				if(callback) { callback(); }
			}
        }];
