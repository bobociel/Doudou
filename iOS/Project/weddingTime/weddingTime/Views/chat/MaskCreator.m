...
-(UIImage*)returnMaskWithImageSize:(CGSize)imageSize outOrIn:(BOOL)ifout
{
    ...
    if (!maskImageDrawnToSize) {
        ...
        width=(int)width;
        height=(int)height;
		originBubble = [originBubble imageByResizeToSize:CGSizeMake(originBubble.size.width*beishu, originBubble.size.height*beishu)];
		...
		maskImageDrawnToSize = [originBubble imageByResizeToSize:CGSizeMake(width, height)];
