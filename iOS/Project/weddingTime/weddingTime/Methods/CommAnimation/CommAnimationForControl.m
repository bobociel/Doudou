...
@implementation CommAnimationScale

+ (POPSpringAnimation*)LayerScaleAnimationDefaultWithCompletionBlock:(void(^)(POPAnimation *anim, BOOL finished))completionBlock{
...
+ (POPSpringAnimation*)LayerScaleAnimationDefault {
    ...
    anim.springBounciness    = defaultLayerScaleSpringBounciness;
    anim.springSpeed         = defaultLayerScalePpringSpeed;
    anim.fromValue           = [NSValue valueWithCGPoint:CGPointMake(0.0, 0.0)];
    anim.toValue             = [NSValue valueWithCGPoint:CGPointMake(1.0, 1.0)];
    return anim;
}
...
+ (POPSpringAnimation*)LayerScaleToScaleWithFromeScale:(float)fscale
                                            andToScale:(float)tscale {
...
@implementation CommAnimationOpacity
...
+ (POPBasicAnimation*)OpacityFromOpacity:(float)fopacity
                               toOpacity:(float)topacity {
...
@implementation CommAnimationCenter

+ (POPSpringAnimation*)CenterDefaultToCenter:(CGPoint)tcenter
                              andBounciness:(float)Bounciness
...
+ (POPSpringAnimation*)CenterDefaultFromeCenter:(CGPoint)fcenter
                                  andBounciness:(float)Bounciness
                                       andSpeed:(float)speed {
...
+ (POPSpringAnimation*)CenterFromeCenter:(CGPoint)fcenter
                               toCenter:(CGPoint)tcenter
                          andBounciness:(float)Bounciness
                               andSpeed:(float)speed {
