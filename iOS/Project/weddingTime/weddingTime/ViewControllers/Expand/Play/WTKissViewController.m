...
- (void)viewDidLoad {
    ...
    [[ChatMessageManager instance]sendToOurConversationBeforeFinish:^(AVIMConversation *ourCov) {
        if (ourCov) {
            [ChatConversationManager sendCustomMessageWithPushName:@"kiss" andConversationTypeKey:ConversationTypeInviteKiss andConversationValue:@"" andCovTitle:_isFromJoin?@"加入了指纹Kiss   " :@"发起指纹kiss   " conversation:ourCov push:YES success:^{
...
- (void)nearViewSet {
    ...
    if (distance<20.f&&patnerImage.isHidden==NO&&myKissImage.isHidden==NO) {
		...
		[[UUAVAudioPlayer sharedInstance] playSongWithData:[UUAVAudioPlayer localAudioDataWithFileName:@"kiss1.mp3"] andType:AVFileTypeMPEGLayer3];
		[[QHAudioPlayer instance] playSoundWithSoundId:QHAudioPlayer_sound_shark];
...
- (void)initView {
    ...
    [self.navigationController.navigationBar setTitleTextAttributes:
     @{NSFontAttributeName:[WeddingTimeAppInfoManager fontWithSize:18],
       ...
       NSForegroundColorAttributeName:LightGragyColor}];
    [self sendPostion:NO];
