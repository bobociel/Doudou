...
- (void)viewDidLoad {
    [super viewDidLoad];
	self.title = [UserInfoManager instance].noPSW ? @"设置密码" : @"修改密码";
    [self initView];
}
...
- (void)done {
	...
	[UserService postChangePasswordWithKey:[UserInfoManager instance].noPSW ? @"new" : @"update"
		...
		{
			NSString *content = [UserInfoManager instance].noPSW ? @"设置密码成功" : @"修改密码成功" ;
			...
			for (UIViewController *VC in self.navigationController.viewControllers) {
				if([VC isKindOfClass:[WTSettingViewController class]]){
					toVC = VC;
					break ;
				}
			}

			toVC ? [self.navigationController popToViewController:toVC animated:NO] : [self.navigationController popToRootViewControllerAnimated:YES] ;
        }
    }];
}
...
- (void)initView {
	...
	textArray = [UserInfoManager instance].noPSW  ? [@[@"输入密码",@"输入确认密码"] mutableCopy] : [@[@"输入原密码",@"输入新密码",@"输入确认密码"] mutableCopy] ;
