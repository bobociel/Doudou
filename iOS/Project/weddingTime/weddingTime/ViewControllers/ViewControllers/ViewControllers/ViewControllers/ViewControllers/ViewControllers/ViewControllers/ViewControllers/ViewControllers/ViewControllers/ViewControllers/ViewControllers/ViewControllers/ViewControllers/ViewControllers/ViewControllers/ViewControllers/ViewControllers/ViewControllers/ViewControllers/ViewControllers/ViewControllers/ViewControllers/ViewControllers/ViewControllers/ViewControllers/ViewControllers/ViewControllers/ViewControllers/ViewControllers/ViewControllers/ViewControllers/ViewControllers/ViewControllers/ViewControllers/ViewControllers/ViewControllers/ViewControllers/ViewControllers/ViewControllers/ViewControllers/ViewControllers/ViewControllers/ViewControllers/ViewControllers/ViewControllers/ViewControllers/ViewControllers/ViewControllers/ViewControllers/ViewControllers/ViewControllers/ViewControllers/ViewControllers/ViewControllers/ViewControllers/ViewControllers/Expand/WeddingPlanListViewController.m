...
- (void)loadDataShowHUD:(BOOL)isShow {
    ...
    [GetService getWeddingPlanListWithStatus:self.status andPage:1 WithBlock:^(NSDictionary *result, NSError *error) {
		...
		{
			NSString *errStr = [LWAssistUtil getCodeMessage:error defaultStr:@"" noresultStr:@"暂时没有数据哦"];
			[WTProgressHUD ShowTextHUD:errStr showInView:_tableView];
			[NetWorkingFailDoErr errWithView:_tableView content:errStr tapBlock:^{
				[self loadData];
			}];
		}
     }];
