...
@property (nonatomic, strong) SquareButton *cardShopSquare;
@property (nonatomic, strong) SquareButton *budgetSquare;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView1{
    ...
    if (yOffset < -50) {
		_budgetSquare.hidden = YES;
		_cardMainSquare.hidden = YES;
		_cardShopSquare.hidden = YES;
	}
	else
	{
        _budgetSquare.hidden = NO;
        _cardMainSquare.hidden = NO;
        _cardShopSquare.hidden = NO;
    }
...
- (void)ourConversationMessageArrive{
	...
	_unReadButton.frame = CGRectMake((screenWidth - rect.size.width) / 2.0, 362 * Height_ato, rect.size.width, 25);
}
...
{
	if([LWAssistUtil isLogin]){
		[self.navigationController pushViewController:[WTBudgetViewController new] animated:YES];
	}
}
...
- (void)setBaseUI
{
	...
	_cardMainSquare.iconImageView.image = [UIImage imageNamed:@"icon_home_expand"];
	...
	_cardShopSquare = [[SquareButton alloc] initWithFrame:CGRectMake(kSquareWidth , kSquareBottomY, kSquareWidth, kSquareHeight)];
	_cardShopSquare.nameLabel.text = @"请柬商店";
	_cardShopSquare.iconImageView.image = [UIImage imageNamed:@"icon_shop_card"];
	[self.view addSubview:_cardShopSquare];
	[_cardShopSquare addTarget:self action:@selector(cardShopAction) forControlEvents:UIControlEventTouchUpInside];

    _budgetSquare = [[SquareButton alloc] initWithFrame:CGRectMake(kSquareWidth * 2,kSquareBottomY, kSquareWidth , kSquareHeight)];
    _budgetSquare.nameLabel.text = @"我的预算";
	_budgetSquare.iconImageView.image = [UIImage imageNamed:@"icon_budget"];
	[self.view addSubview:_budgetSquare];
    [_budgetSquare addTarget:self action:@selector(budgetAction) forControlEvents:UIControlEventTouchUpInside];
}
...
- (void)setUIWithBling
{
	...
	_unReadButton.frame = CGRectMake((screenWidth - 80) / 2.0, 362 * Height_ato, 80 , 25);
