...
+ (instancetype)instanceWTBindingViewControllerWithToken:(NSString *)token
{
...
- (IBAction)next
{
    ...
    [UserService bindPhoneWthPhone:_phoneTextField.text  andConfrimCode:_verifyTextFidld.text withToken:_token withBlock:^(NSDictionary *result, NSError *error) {
...
- (void)doregisterFinishWithResult:(NSDictionary *)result andError:(NSError *)err {
    if (err||!result||![result isKindOfClass:[NSDictionary class]]) {
		...
		{
			[self.navigationController pushViewController:[WTChangePasswordViewController instanceVCWithToken:_token] animated:YES];
		}
    }
}
