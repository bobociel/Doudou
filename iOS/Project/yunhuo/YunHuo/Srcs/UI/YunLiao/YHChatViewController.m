...
@interface YHChatViewController ()<UITableViewDataSource, UITableViewDelegate, UINavigationControllerDelegate, UIImagePickerControllerDelegate, SRRefreshDelegate, IChatManagerDelegate, DXChatBarMoreViewDelegate, DXMessageToolBarDelegate, IDeviceManagerDelegate /*,LocationViewDelegate*/>
{
...
@property (nonatomic) NSMutableDictionary	*audioCellExpandStatus;
...
- (void)viewDidLoad
{
	...
	self.view.backgroundColor = [UIColor colorWithRed:239/255.0 green:239/255.0 blue:244/255. alpha:1.0];
	
	self.audioCellExpandStatus = [NSMutableDictionary dictionary];
	_isPlayingAudio = NO;
	//TBD
	NSString *im_id = @"Test";//[self.chatTarget imID];
	_conversation = [[EaseMob sharedInstance].chatManager conversationForChatter:im_id isGroup:NO];
...
- (void)viewWillAppear:(BOOL)animated
{
	...
	//TBD
	self.navigationItem.title = @"张三";//[self.chatTarget userName];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	if (indexPath.row < [self.dataSource count]) {
		...
		if ([obj isKindOfClass:[NSString class]]) {
			...
			cell.isShowTranslation = [self.audioCellExpandStatus[@(indexPath.row)] integerValue] != 0;
			cell.messageModel = model;
			
			//TBD
			cell.headImageView.image = [UIImage imageNamed:@"DefaultAvatar"];//model.isSender?[FYConfig instance].user.userInfo.photo:[self.chatTarget photo];
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if ([obj isKindOfClass:[NSString class]]) {
		...
		//for audio translate
		if ( ((MessageModel*)obj).type == eMessageBodyType_Voice )
		{
			if ( [self.audioCellExpandStatus[@(indexPath.row)] integerValue] == 1 )
			{
				//has translate
				return [EMChatViewCell tableView:tableView heightForRowAtIndexPath:indexPath withObject:(MessageModel *)obj] + 100;
			}
			else
			{
...
- (void)routerEventWithName:(NSString *)eventName userInfo:(NSDictionary *)userInfo
{
	...
	/*if ([eventName isEqualToString:kRouterEventMSFYBubbleTapEventName]) {
		if ( [FYProduct isValidIMStr:model.content] )
		{
			//跳转到产品详情页面
#if MANAGER_VERSION == 1
			FYManagerProductDetailViewController *productVC = [FYSceneManager loadManagerProductDetail];
#else
			FYProductDetailViewController *productVC = [FYSceneManager loadProductDetail];
#endif
			productVC.product = [FYProduct productWithIMStr:model.content];
	...
	}else if([eventName isEqualToString:kRouterEventAudioBubbleShowTranslationEventName]){
		
		EMChatViewCell *cell = [userInfo objectForKey:kShowTranlationCell];
//		MessageModel *messageModel = cell.messageModel;
		NSIndexPath *indexPath = [self.tableView indexPathForCell:cell];
		
		if ( indexPath != nil )
		{
			self.audioCellExpandStatus[@(indexPath.row)] = @1;
	...
	}else if([eventName isEqualToString:kRouterEventAudioBubbleHideTranslationEventName]){
		EMChatViewCell *cell = [userInfo objectForKey:kShowTranlationCell];
		//		MessageModel *messageModel = cell.messageModel;
		NSIndexPath *indexPath = [self.tableView indexPathForCell:cell];
		...
		if ( indexPath != nil )
		{
			self.audioCellExpandStatus[@(indexPath.row)] = @0;
...
-(void)chatAudioCellBubblePressed:(MessageModel *)model
{
	...
	if (model.type == eMessageBodyType_Voice) {
		__weak YHChatViewController *weakSelf = self;
...
//	NSArray *menuItems =
//	@[
//	  [KxMenuItem menuItem:@"理财产品" image:[UIImage imageNamed:@"IconAddAdvisor"] target:self action:@selector(shareProduct)],
//	  [KxMenuItem menuItem:@"理财顾问" image:[UIImage imageNamed:@"IconAddFriend"] target:self action:@selector(shareAdvisor)],
//	  ];
...
//- (void) shareProduct
//{
//	[self performSegueWithIdentifier:@"showProductList" sender:nil];
//}
//
//- (void) shareAdvisor
//{
//	[self performSegueWithIdentifier:@"showAdvisorList" sender:nil];
//}
//
//- (void) showProductDetail:(NSString*)product
//{
//	[self performSegueWithIdentifier:@"showProductDetail" sender:product];
//}
//
//- (void) prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
//{
//	if ( [segue.identifier isEqualToString:@"showAdvisorList"] )
//	{
//		
//		NSInvocation *invocation = [NSInvocation invocationWithMethodSignature:[self methodSignatureForSelector:@selector(setManagerToShare:)]];
//		[invocation setTarget:self];
//		[invocation setSelector:@selector(setManagerToShare:)];
//		((FYAdvisorsViewController*)segue.destinationViewController).selectedAction = invocation;
//	}
//	else if ( [segue.identifier isEqualToString:@"showProductList"] )
//	{
//		NSInvocation *invocation = [NSInvocation invocationWithMethodSignature:[self methodSignatureForSelector:@selector(setProductToShare:)]];
//		[invocation setTarget:self];
//		[invocation setSelector:@selector(setProductToShare:)];
//		((FYAllProductsListViewController*)segue.destinationViewController).selectedAction = invocation;
//	}
//	else if ( [segue.identifier isEqualToString:@"showProductDetail"] )
//	{
//		((FYProductDetailViewController*)segue.destinationViewController).product = sender;
//	}
//}
@end
