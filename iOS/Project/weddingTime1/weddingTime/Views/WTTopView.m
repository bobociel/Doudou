...
@property (weak, nonatomic) IBOutlet UIButton *setBGButton;
...
@property (weak, nonatomic) IBOutlet UIControl *filterControl;
@property (weak, nonatomic) IBOutlet UIControl *searchControl;
...
+ (instancetype)topViewInView:(UIView *)superView withType:(NSArray *)types
{
	...
	view.frame = CGRectMake(0, 0 , screenWidth, kTopViewHeight);
	if(types)
	{
...
- (void)setUnreadCount:(double)unreadCount
{
	...
	_unreadLabel.textColor = _unreadCount <= 0 ? rgba(153, 153, 153, 1) : WeddingTimeBaseColor ;
}
...
{
	if (![LWAssistUtil isLogin] || (!_hotel_id && !_supplier_id)) { return; }
