...
- (void)loadData {
    [self changeListWithStatus:PPlanStatusAll];
}

- (void)changeListWithStatus:(int)status {
    if (status == 0) {
        if (!weddingPlanListViewControllerAll) {
            weddingPlanListViewControllerAll =
            [[WeddingPlanListViewController alloc] init];
            weddingPlanListViewControllerAll.status = PPlanStatusAll;
            [self addChildViewController:weddingPlanListViewControllerAll];
            weddingPlanListViewControllerAll.view.frame = CGRectMake(0, 110-64, screenWidth, screenHeight-110+64);
        }
        ...
        if (!weddingPlanListViewControllerFinish) {
            weddingPlanListViewControllerFinish =
            [[WeddingPlanListViewController alloc] init];
            weddingPlanListViewControllerFinish.status = PPlanStatusFinish;
            [self addChildViewController:weddingPlanListViewControllerFinish];
            weddingPlanListViewControllerFinish.view.top = 110-64;
            weddingPlanListViewControllerFinish.view.height = screenHeight-110;
        }
        ...
        if (!weddingPlanListViewControllerUnFinish) {
            weddingPlanListViewControllerUnFinish =
            [[WeddingPlanListViewController alloc] init];
            weddingPlanListViewControllerUnFinish.status = PPlanStatusNotFinish;
            [self addChildViewController:weddingPlanListViewControllerUnFinish];
            weddingPlanListViewControllerUnFinish.view.top = 110-64;
            weddingPlanListViewControllerUnFinish.view.height = screenHeight-110;
        }
...
- (void)initView {
    ...
    inviteSegmentedControl.top=75-64;
