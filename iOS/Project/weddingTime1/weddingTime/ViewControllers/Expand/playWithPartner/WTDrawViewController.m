...
- (void)viewDidLoad {
    ...
    [[ChatMessageManager instance]sendToOurConversationBeforeFinish:^(AVIMConversation *ourCov) {
        if (ourCov) {
            [ChatConversationManager sendCustomMessageWithPushName:nil andConversationTypeKey:ConversationTypeInviteDraw andConversationValue:@"" andCovTitle:self.isFromJoin ? @"加入了一起画"
...
+(void)pushViewControllerWithRootViewController:(UIViewController*)root isFromJoin:(BOOL)isFromJoin isFromChat:(BOOL)isFromChat
{
    ...
    WTDrawViewController *next=[[UIStoryboard storyboardWithName:@"Main" bundle:nil] instantiateViewControllerWithIdentifier:@"WTDrawViewController"];
