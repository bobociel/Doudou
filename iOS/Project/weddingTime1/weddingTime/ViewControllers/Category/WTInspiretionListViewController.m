    ...
    if (errorType==NetWorkStatusTypeNone) {
        if (self.showWithLike) {
            ...
            for (int i=0;i<array.count; i++) {
                ...
                [mDic setObject:@"1" forKey:@"is_like"];//妈蛋喜欢列表不返回is_like字段
        ...
        if ([self.items count]==0) {
            //            [WTProgressHUD ShowTextHUD:@"暂时没有灵感哦" showInView:self.view];
...
- (void)initRefresh
{
    __weak UIScrollView *scrollView = self.dataCollectionView;
...
-(void)initView
{
    ...
    self.dataCollectionView = [[PSCollectionView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height)];
    ...
    self.dataCollectionView.autoresizingMask = UIViewAutoresizingFlexibleLeftMargin |
...
-(void)networkImageShow:(NSUInteger)index{
    [PhotoBroswerVC show:self needFav:YES type:PhotoBroswerVCTypePush index:index photoModelBlock:^NSArray *{
        ...
        for (NSUInteger i = 0; i< networkImages.count; i++) {
            ...
            pbModel.desc = [NSString stringWithFormat:@"来自 @%@",self.items[i][@"name"]];
            pbModel.image_HD_U = networkImages[i];
			pbModel.postID = @([self.items[i][@"post_id"] integerValue]);
