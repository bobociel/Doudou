...
#define QUERY_MSG_SQL @"SELECT * from msgs where convid=? and timestamp<? order by timestamp desc limit ?"
...
- (void)pullMessagesForConversationid:(NSString*)conversationid preceded:(NSString*)lastMessageId timestamp:(int64_t)timestamp limit:(int)limit callback:(ArrayResultBlock)block{
    
    [_dbQueue inDatabase:^(FMDatabase *db) {
        FMResultSet* rs = [db executeQuery:QUERY_MSG_SQL
                      withArgumentsInArray:@[conversationid,@(timestamp), @(limit)]];
