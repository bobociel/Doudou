    ...
    if (![_message.imMessage isKindOfClass:[AVIMTypedMessage class]]) {
        ...
        switch (typedMessage.mediaType) {
                ...
                if (!typedMessage.attributes) {
                    ...
                    if ([conversationTypeKey isEqualToString:ConversationTypeSendFaceImage]) {
                        ...
                        contentSize = CGSizeMake(image.size.width>0&&image.size.width<ChatPicWH?image.size.width:ChatPicWH, image.size.height>0&&image.size.height<ChatPicWH?image.size.height:ChatPicWH);
							 ...
							 &&![conversationTypeKey isEqualToString:ConversationTypeInvitePlan])
                    {
                        NSString *contentText=[NSString stringWithFormat:@"%@（当前版本不支持此功能）",((AVIMTextMessage*)message.imMessage).text];
            ...
            case kAVIMMessageMediaTypeImage: {
                ...
                CGSize imageSize=CGSizeMake(ChatPicWH, ChatPicWH);
                ...
                imageSize=CGSizeMake(picwidth, picheight);
...
- (BOOL)showRightImage:(NSDictionary *)attributes {
    if ([attributes[ConversationTypeKey] isEqualToString:ConversationTypeLike]
        ||[attributes[ConversationTypeKey] isEqualToString:ConversationTypeInviteKiss]
        ||[attributes[ConversationTypeKey] isEqualToString:ConversationTypeInviteDraw]
        ||[attributes[ConversationTypeKey] isEqualToString:ConversationTypeChatWithSupplier]
        ||[attributes[ConversationTypeKey] isEqualToString:ConversationTypeCreateOrder]
        ||[attributes[ConversationTypeKey] isEqualToString:ConversationTypeCreateDemand])  {
        return YES;
    }else {
        return NO;
    }
