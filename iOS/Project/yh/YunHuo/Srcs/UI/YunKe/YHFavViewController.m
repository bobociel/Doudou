...
#import "YHFavViewController.h"

@implementation YHFavCell

- (void) awakeFromNib
{
	self.selectMark.hidden = YES;
}

- (void) setIsSelected:(BOOL)isSelected
{
	_isSelected = isSelected;
	self.selectMark.highlighted = isSelected;
}

- (void) setIsSelectableMode:(BOOL)isSelectableMode
{
	self.selectMark.hidden = !isSelectableMode;
}

@end

@interface YHFavViewController ()<UITableViewDataSource,UITableViewDelegate,UIActionSheetDelegate,UINavigationControllerDelegate,UIImagePickerControllerDelegate,UITextFieldDelegate>
@property (nonatomic) NSMutableArray	*selectStatus;

@end

@implementation YHFavViewController
...
//		[self.tabBarController.tabBar setHidden:NO];
		[UIView animateWithDuration:0.25 animations:^{
			self.addBtn.center = CGPointMake(CGRectGetMidX(self.view.bounds),CGRectGetMaxY(self.view.bounds) - 40);
			self.editToolbar.center = CGPointMake(CGRectGetMidX(self.view.bounds),CGRectGetMaxY(self.view.bounds) + 22);
		}];
	}
