...
+ (void)saveMediaWith:(NSMutableDictionary *)mediaInfo callback:(void(^)(BOOL saved,NSString *filePath))callback
{
	NSString *filePath = [[[mediaInfo[UIImagePickerControllerMediaURL] absoluteString] componentsSeparatedByString:@"file://"] lastObject];
...
- (void)resumeUploadCache{
    ...
    if(files.count>0){
        ...
        self.fileKey = atts[1];
        [self uploadFileWithFileInfo:self.filePath fileType:WTFileTypeVideo fileBucket:@"mt-video"];
    }
}
