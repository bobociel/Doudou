...
//			if(isStop){ [self.navigationController popViewControllerAnimated:YES]; }
		}];
	}
...
- (void)loadDataShowHUD:(BOOL)isShow
{
    ...
    [GetService getHomePageStoryWithPage:1 andMediaType:@"image" WithBlock:^(NSDictionary *result, NSError *error) {
		[self hideLoadingView];
        if (!error) {
			if(!isShow) { photoStoryList = [NSMutableArray array]; }
...
- (UITableViewCellEditingStyle)tableView:(UITableView *)tableView editingStyleForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	//dataTableView.isEditing ? UITableViewCellEditingStyleNone : UITableViewCellEditingStyleDelete;
}
...
{
    if(!indexPath || photoStoryList.count == 0) { return ; }
	...
	[PostDataService postWeddingHomeUpadateStoryWithStory:uploadStory orStorys:(NSArray *)uploadStorys withBlock:^(NSDictionary *result, NSError *error) {
		...
		if (!error) {
			if(SegmentTypePhoto == currentSegmentIndex){
				NSArray *storyArray = [NSArray modelArrayWithClass:[WTWeddingStory class] json:result[@"data"]];
				[photoStoryList insertObjects:storyArray atIndex:0];
				[dataTableView reloadData];
			}else{
				videoStory = [WTWeddingStory modelWithJSON:result[@"data"]];
				[dataTableView reloadData];
			}
