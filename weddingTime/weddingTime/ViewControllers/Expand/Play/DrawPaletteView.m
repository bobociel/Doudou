...
- (void)startSendWithLineArr:(NSArray *)lines andIndex:(int)index andHash:(int)hash andTotalCount:(int)totalCount{
    ...
    AVIMTextMessage *avMessage = [AVIMTextMessage messageWithText:@"draw" attributes:@{ConversationTypeKey:ConversationTypeDraw,ConversationIsForGameKey:@(YES),@"data":@{@"isClean":@(NO),@"line":[sendLine copy],@"color":myallColor[index],@"width":myallwidth[index],@"hash":@(hash),@"total":@(totalCount),@"index":@(index)}}];
...
- (void)sendCleanMyDraw {
    ...
    AVIMTextMessage *avMessage = [AVIMTextMessage messageWithText:@"draw" attributes:@{ConversationTypeKey:ConversationTypeDraw,ConversationIsForGameKey:@(YES),@"data":@{@"isClean":@(YES)}}];
