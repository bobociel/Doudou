...
- (void)viewDidLoad {
    ...
    [normalScanBtn setBackgroundColor:[UIColor orangeColor]];
    ...
    [customScanBtn setBackgroundColor:[UIColor orangeColor]];
    ...
    [createBtn setBackgroundColor:[UIColor orangeColor]];
	...
	[codeBtn setTitle:@"系统二维码" forState:UIControlStateNormal];
	[codeBtn setBackgroundColor:[UIColor orangeColor]];
	[codeBtn addTarget:self action:@selector(systemQRCodeView) forControlEvents:UIControlEventTouchUpInside];
	[self.view addSubview:codeBtn];
	[codeBtn mas_makeConstraints:^(MASConstraintMaker *make) {
		make.centerX.equalTo(weakSelf.view);
		make.top.equalTo(weakSelf.view).with.offset(400);
		make.size.mas_equalTo(CGSizeMake(150, 40));
	}];

	codeBtn.frame = CGRectMake(([UIScreen mainScreen].bounds.size.width-150)/2, 400, 150, 40);
}
...
- (void)systemQRCodeView
{
	[self.navigationController pushViewController:[SystemCodeViewController new] animated:YES];
}
