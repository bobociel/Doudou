...
@property (strong, nonatomic) UIButton *recordAtButton;
@property (strong, nonatomic) UIButton *showRecordViewBtn;
/**
 *  底部扩展页面
 */
...
- (void)setupSubviews
{
	...
	//排序按钮
//	self.sortButton = [[UIButton alloc] initWithFrame:CGRectMake(kHorizontalPadding, kVerticalPadding, kInputTextViewMinHeight, kInputTextViewMinHeight)];
//	self.sortButton.autoresizingMask = UIViewAutoresizingFlexibleTopMargin;
//	[self.sortButton setImage:[UIImage imageNamed:@"chatBar_sort"] forState:UIControlStateNormal];
//	[self.sortButton addTarget:self action:@selector(buttonAction:) forControlEvents:UIControlEventTouchUpInside];
//	self.sortButton.tag = 3;
//	allButtonWidth += CGRectGetMaxX(self.sortButton.frame);
//	textViewLeftMargin += CGRectGetMaxX(self.sortButton.frame);
	
    //转变输入样式
    self.styleChangeButton = [[UIButton alloc] initWithFrame:CGRectMake(CGRectGetMaxX(self.sortButton.frame) + kHorizontalPadding, kVerticalPadding, kInputTextViewMinHeight, kInputTextViewMinHeight)];
...
//    allButtonWidth += CGRectGetWidth(self.styleChangeButton.frame) + kHorizontalPadding * 2.5;
//    textViewLeftMargin += CGRectGetWidth(self.styleChangeButton.frame);
...
//    self.inputTextView.contentMode = UIViewContentModeCenter;
    ...
    _inputTextView.backgroundColor = [UIColor colorWithWhite:250/255.0 alpha:1.0];
    ...
    //录制
	self.showRecordViewBtn = [[UIButton alloc] initWithFrame:CGRectMake(textViewLeftMargin, kVerticalPadding, kInputTextViewMinHeight, kInputTextViewMinHeight)];
	[self.showRecordViewBtn setImage:[UIImage imageNamed:@"chatBar_down"] forState:UIControlStateNormal];
	[self.showRecordViewBtn setImage:[UIImage imageNamed:@"chatBar_up"] forState:UIControlStateSelected];
	self.showRecordViewBtn.hidden = YES;
	[self.showRecordViewBtn addTarget:self action:@selector(showRecordView) forControlEvents:UIControlEventTouchUpInside];
	self.showRecordViewBtn.hidden = YES;
	
	self.recordAtButton = [[UIButton alloc] initWithFrame:CGRectMake(textViewLeftMargin+kInputTextViewMinHeight+kHorizontalPadding, kVerticalPadding, kInputTextViewMinHeight, kInputTextViewMinHeight)];
	self.recordAtButton.showsTouchWhenHighlighted = YES;
//	self.recordAtButton.backgroundColor = [UIColor blueColor];
//	self.recordAtButton.titleLabel.font = [UIFont systemFontOfSize:15.0];
//	[self.recordAtButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
	[self.recordAtButton setImage:[UIImage imageNamed:@"chatBar_at"] forState:UIControlStateNormal];
	
//	[self.recordAtButton setBackgroundImage:[[UIImage imageNamed:@"chatBar_recordBg"] stretchableImageWithLeftCapWidth:10 topCapHeight:10] forState:UIControlStateNormal];
//	[self.recordAtButton setBackgroundImage:[[UIImage imageNamed:@"chatBar_recordSelectedBg"] stretchableImageWithLeftCapWidth:10 topCapHeight:10] forState:UIControlStateHighlighted];
//	[self.recordAtButton setTitle:@"@" forState:UIControlStateNormal];
//	[self.recordAtButton setTitle:kTouchToFinish forState:UIControlStateHighlighted];
	self.recordAtButton.hidden = YES;
	[self.recordAtButton addTarget:self action:@selector(recordAtButtonTouchUpInside) forControlEvents:UIControlEventTouchUpInside];
	self.recordAtButton.hidden = YES;
    ...
    if (!self.moreView) {
        ...
        self.moreView.backgroundColor = [UIColor colorWithWhite:250/255.0 alpha:1.0];
    ...
    if (!self.faceView) {
        ...
        self.faceView.backgroundColor = [UIColor colorWithWhite:250/255.0 alpha:1.0];
	...
	if (!self.recordBtnView) {
		self.recordBtnView = [[UIView alloc] initWithFrame:CGRectMake(0, (kVerticalPadding * 2 + kInputTextViewMinHeight), self.frame.size.width, 180)];
//		self.recordBtnView.backgroundColor = [UIColor colorWithWhite:250/255.0 alpha:1.0];
		self.recordBtnView.backgroundColor = [UIColor whiteColor];
		self.recordBtnView.autoresizingMask = UIViewAutoresizingFlexibleTopMargin;

		UIButton *recordButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 150, 150)];
//		recordButton.backgroundColor = [UIColor greenColor];
		recordButton.titleLabel.font = [UIFont systemFontOfSize:15.0];
		[recordButton setTitleColor:[UIColor darkGrayColor] forState:UIControlStateNormal];
		[recordButton setImage:[UIImage imageNamed:@"VoiceRecordBtn"] forState:UIControlStateNormal];
//		[recordButton setBackgroundImage:[[UIImage imageNamed:@"chatBar_recordBg"] stretchableImageWithLeftCapWidth:10 topCapHeight:10] forState:UIControlStateNormal];
//		[recordButton setBackgroundImage:[[UIImage imageNamed:@"chatBar_recordSelectedBg"] stretchableImageWithLeftCapWidth:10 topCapHeight:10] forState:UIControlStateHighlighted];
...
//		[recordButton setTitle:kTouchToFinish forState:UIControlStateHighlighted];
		...
		recordButton.center = CGPointMake(CGRectGetMidX(self.recordBtnView.bounds),CGRectGetMidY(self.recordBtnView.bounds));
		[self.recordBtnView addSubview:recordButton];
	}
	
	[self.toolbarView addSubview:self.sortButton];
    ...
    [self.toolbarView addSubview:self.recordAtButton];
    [self.toolbarView addSubview:self.showRecordViewBtn];
}
    ...
    switch (tag) {
        case 0://切换状态
        {
            if (button.selected) {
                self.faceButton.selected = NO;
                self.moreButton.selected = NO;
				self.showRecordViewBtn.hidden = NO;
				self.showRecordViewBtn.selected = NO;
                [self.inputTextView resignFirstResponder];
				
                //录音状态下，显示底部录音扩展页面
                [self willShowBottomView:self.recordBtnView];
            ...
            [UIView animateWithDuration:0.2 delay:0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
                self.recordAtButton.hidden = !button.selected;
...
- (void) recordAtButtonTouchUpInside
{
	//选择@的目标
	if (_delegate && [_delegate respondsToSelector:@selector(didAtAction)]) {
		[_delegate didAtAction];
	}
}

- (void) showRecordView
{
	self.showRecordViewBtn.selected = !self.showRecordViewBtn.selected;
	if ( self.showRecordViewBtn.selected )
	{
		[self endEditing:YES];		
	}
	else
