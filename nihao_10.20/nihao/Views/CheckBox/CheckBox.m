...
{
    [self setSelected:!self.selected distinct:YES sendControlEvent:YES];
}

-(void) setGroupButtons:(NSArray *)buttons
{
    if(!_sharedLinks) {
        for(CheckBox* rb in buttons) {
            if(rb->_sharedLinks) {
    ...
    BOOL (^btnExistsInList)(NSArray*, CheckBox*) = ^(NSArray* list, CheckBox* rb){
        ...
        if(rb->_sharedLinks!=_sharedLinks) {
            if(!rb->_sharedLinks) {
                ...
                for(NSValue* v in rb->_sharedLinks) {
                    CheckBox* vrb = [v nonretainedObjectValue];
...
-(CheckBox*) selectedButton
{
    if([self isSelected]) {
        return self;
    } else {
        for(NSValue* v in _sharedLinks) {
            CheckBox* rb = [v nonretainedObjectValue];
            if([rb isSelected]) {
                return rb;
            }
        }
...
-(void) setButtonSelected:(BOOL)selected sendControlEvent:(BOOL)sendControlEvent
{
    ...
    if ([_delegate respondsToSelector:@selector(checkBox:didChangedSelectedStatus:)]) {
        [_delegate checkBox:self didChangedSelectedStatus:selected];
    }
}
