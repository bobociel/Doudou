...
- (void)viewDidLoad {
    ...
    UILongPressGestureRecognizer *longPressGesture = [[UILongPressGestureRecognizer alloc]initWithTarget:self action:@selector(longPressDected:)];
...
- (void)image:(UIImage *)image didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo {
    [self showProgressHUDCompleteMessage: error ? NSLocalizedString(@"Failed", @"Informing the user a process has failed") : NSLocalizedString(@"Saved", @"Saved to album")];
    [self hideControlsAfterDelay]; // Continue as normal...
}
...
- (void) longPressDected : (UILongPressGestureRecognizer *) recognizer {
    ...
    _savePicSheet = [[UIActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:@"Cancel" destructiveButtonTitle:@"Save Photo" otherButtonTitles:nil, nil];
    [_savePicSheet showInView:self.view];
}
...
- (void) saveToAlbum {
    ...
    //保存图片
    ALAssetsLibrary *library = [MWPhotoBrowser defaultAssetsLibrary];
    UIImage *img = [UIImage imageWithData:[NSData dataWithContentsOfURL:photo.photoURL]];
