...
-(void)loadFacialView:(int)page size:(CGSize)size {
	int maxRow = roundf(_faces.count / 8.0);
	int maxCol = 8;
	CGFloat itemWidth = self.frame.size.width / maxCol;
	CGFloat itemHeight = self.frame.size.height / 5;
	
	UIScrollView *scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth(self.frame), CGRectGetHeight(self.frame) - itemHeight)];
	CGFloat contentHeight = maxRow * itemHeight;
	scrollView.contentSize = CGSizeMake(CGRectGetWidth(self.frame), contentHeight);
	scrollView.backgroundColor = FaceBGColor;
	[self addSubview:scrollView];
	
	UIButton *deleteButton = [UIButton buttonWithType:UIButtonTypeCustom];
	[deleteButton setBackgroundColor:[UIColor clearColor]];
	[deleteButton setFrame:CGRectMake((maxCol - 1) * itemWidth, 4 * itemHeight, itemWidth, itemHeight)];
	...
	[sendButton setFrame:CGRectMake((maxCol - 2) * itemWidth - 10, 4 * itemHeight + 5, itemWidth + 10, itemHeight - 10)];
