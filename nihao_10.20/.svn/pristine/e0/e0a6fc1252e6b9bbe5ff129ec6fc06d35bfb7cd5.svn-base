...
#import "BackCommentToolbar.h"

@interface BackCommentToolbar () <UITextViewDelegate, DXFaceDelegate>{
...
@implementation BackCommentToolbar
...
- (void)setupSubviews {
	...
	//	self.backBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(self.toolBar.frame) - 34 - 5, 40, 34)];
	//	[self.backBtn setImage:[UIImage imageNamed:@"icon_back_news"] forState:UIControlStateNormal];
	//	[self.backBtn setImageEdgeInsets:UIEdgeInsetsMake(7, 10, 7, 10)];
	//	[self.backBtn addTarget:self action:@selector(backBtnClick) forControlEvents:UIControlEventTouchUpInside];
	//	[self.toolBar addSubview:self.backBtn];
	
	// 返回按钮
	self.backButton = [[UIButton alloc] initWithFrame:CGRectMake(kHorizontalPadding, kVerticalPadding, kInputTextViewMinHeight, kInputTextViewMinHeight)];
	self.backButton.autoresizingMask = UIViewAutoresizingFlexibleTopMargin;
	[self.backButton setImage:[UIImage imageNamed:@"icon_back_news"] forState:UIControlStateNormal];
	self.backButton.imageEdgeInsets = UIEdgeInsetsMake(7, 8, 7, 8);
	...
	allButtonWidth += CGRectGetMaxX(self.backButton.frame);
	textViewLeftMargin += CGRectGetMaxX(self.backButton.frame);
	
//	//表情
//	self.faceButton = [[UIButton alloc] initWithFrame:CGRectMake(CGRectGetWidth(self.bounds) - kInputTextViewMinHeight - kHorizontalPadding, kVerticalPadding, kInputTextViewMinHeight, kInputTextViewMinHeight)];
...
//	allButtonWidth += CGRectGetWidth(self.faceButton.frame) + kHorizontalPadding * 1.5;
	...
	// 初始化输入框
	self.inputTextView = [[XHMessageTextView  alloc] initWithFrame:CGRectMake(textViewLeftMargin, kVerticalPadding, width - kHorizontalPadding * 2, kInputTextViewMinHeight)];
	...
	_inputTextView.placeHolder = self.hintText;
		...
		switch (tag) {
			case 1: {// 返回
				if (_delegate && [_delegate respondsToSelector:@selector(didClickedBackButton)]) {
					[_delegate didClickedBackButton];
				}
			}
				break;
			default:
...
//	self.inputTextView.placeHolder = hintText;
//}
